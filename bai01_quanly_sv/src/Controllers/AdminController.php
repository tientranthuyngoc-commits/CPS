...
class AdminController
{
    ...
    public function userForm(): void
    {
        ...
        $item = null;
        if ($id) {
            $st = $pdo->prepare('SELECT id, username, email, phone, role, is_active, block_reason FROM users WHERE id=:id');
            $st->execute([':id'=>$id]);
    ...
    public function userSave(): void
    {
        ...
        if ($id) {
            if ($password !== '') {
                ...
                $st = $pdo->prepare('UPDATE users SET username=:u, email=:e, phone=:p, role=:r, is_active=:a, block_reason=:b, password_hash=:h WHERE id=:id');
                $st->execute([':u'=>$username, ':e'=>$email, ':p'=>$phone, ':r'=>$role, ':a'=>$isActive, ':b'=>$blockReason, ':h'=>$hash, ':id'=>$id]);
            } else {
