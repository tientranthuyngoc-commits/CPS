...
// Ensure session started and force logout immediately if account was blocked
...
if (!empty($_SESSION['user_id'])) {
  try {
    ...
    if ($u && !(int)$u['is_active']) {
      // Redirect to login with reason; destroy session so user is logged out
      $reason = $u['block_reason'] ? $u['block_reason'] : 'Không có lý do được cung cấp';
      ...
      @session_destroy();
      // Pass reason via query string (URL-encoded)
      header('Location: index.php?action=login&blocked=1&reason=' . urlencode($reason));
      exit;
    }
  } catch (\Throwable $e) {
    // If DB check fails, don't block rendering the site; fail open
  }
}
